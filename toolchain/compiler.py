...
def compile(logger, files, output):
  out = common.run_command(GCC, ["-S", "-Xassembler", "-ahlsm", "-mxl-soft-div", "-mhard-float", "-mxl-soft-mul", "-fno-delayed-branch", "-flive-range-shrinkage", "-ffixed-13", "-ffixed-31", "-ffixed-30", "-O2"] + ["-o", output] + files)
  logger.debug("GCC: " + out[0])
